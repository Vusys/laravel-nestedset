...
- A direct `DB::table('categories')->update(['lft' => …, 'rgt' => …])`
...
1. Pick one row in the cycle and `update categories set parent_id = null`
...
- Direct `DB::table('categories')->update(['articles' => …])` that bypassed
...
[Repairing Aggregates](fix-aggregates.html) for routing options on
the queued form, chunked self-redispatch, and deferred maintenance.
...
3. **Lost source values.** If the column an aggregate is computed
   over (e.g. `articles`) has itself been corrupted, `fixAggregates`
...
*See also: [Tree Repair](fix-tree.html) for the public API surface
