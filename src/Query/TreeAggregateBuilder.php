...
final class TreeAggregateBuilder
{
    ...
    private static function selectStoredAndComputedViaChainFold(
        ...
        foreach ($definitions as $definition) {
            ...
            // Parallel accumulators for the derived-from-companions
            // kinds (AVG, WeightedAvg, BoolOr, BoolAnd): each needs a
            // pair of numerators / denominators that a single int|float
            // fold can't carry. AVG: SUM(value) + COUNT(non-null).
            // WeightedAvg: SUM(weight·value) + SUM(weight).
            // Bool kinds: SUM(asInt) + COUNT(non-null).
            $prevSumAvg = 0.0;
            $prevCountAvg = 0;
            $prevSumWx = 0.0;
            $prevSumW = 0.0;
            ...
            foreach ($rows as $row) {
                ...
                $currentSum = $prevSumAvg;
                $currentCount = $prevCountAvg;
    ...
    private static function chainFoldStep(
        ...
        switch ($definition->function) {
            ...
            case AggregateFunction::BoolAnd:
                // Derived-from-companions kinds — AVG, WeightedAvg,
                // BoolOr, BoolAnd — are handled inline by the caller
                // because each needs two parallel accumulators
                // (sum + count, or sum-of-products + sum-of-weights)
