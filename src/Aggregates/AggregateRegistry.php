        ...
        foreach ($definitions as $definition) {
            if ($definition instanceof AggregateDefinition) {
                ...
                foreach ($companionSet as $spec) {
                    ...
                    // expression in the SUM (e.g. `weight * value` for
                    // weightedAvg, `CASE WHEN bool THEN 1 ELSE 0 END`
                    // for boolOr/boolAnd). A user-declared plain `Sum`
                    // on the same column means something different —
                    // never adopt it. Always create an internal companion.
